...
folder_name = f'../Figures/Results/MLP/{forecast_periods} months'
...
for maturity in maturities:
    ...
    model = keras.Sequential([
        layers.Input(shape=(forecast_periods,)),
    ...
    if forecast_periods > 3:
        ...
        plt.plot(test_dates, y_pred, label='MLP Forecast', linestyle='--', marker='o', color="C0")
...
plt.figure(figsize=(10, 6))
plt.plot(predicted_df.columns, predicted_df.loc[last_date], marker='o', label=f'MLP Forecasted Yield Curve on {last_date}', color='C0')
...
results_df_rounded = results_df.round(6)
...
def summary_table(dfx):
    ...
    summary_df_rounded = summary_df.round(6)
